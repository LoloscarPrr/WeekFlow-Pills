...
export function recordIntake(occurrence: DoseOccurrence, status: IntakeStatus) {
  ensureDatabase();
  const existing = db.getFirstSync<{ status: IntakeStatus }>(
    `SELECT status FROM intakes
  ...
  if (occurrence.medication.stock !== null && existing?.status !== status) {
